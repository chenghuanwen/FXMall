...
/** 等比例缩放充满整个控件的imageview
 * Created by Android004 on 2017/5/10.
 */

public class FullImageView extends ImageView implements OnGlobalLayoutListener{
    ...
    public void onGlobalLayout() {
        if (once) {// 初始化操作，只需要搞一次即可。但是onGlobalLayout 会被多次调用
            ...
            if (drawable == null)
                return;
            if (DEBUG) {
                LogUtil.i("TAGdrawable: " ,""+ drawable);
            }
            ...
            if (DEBUG) {
                LogUtil.i("TAGdw: ","" + dw);
                LogUtil.i("TAGdh: ","" + dh);
                LogUtil.i("TAGwidth: ","" + width);
                LogUtil.i("TAGheight: ","" + height);
            }
