...
public class FXMallModel {
                ...
                if (result.contains("1000")) {
                    try {
                        ...
                        for (int i = 0; i < dataset.length(); i++) {
                            ...
                            //TODO 根据佣金计算会员价
                            product.setVipPrice(product.getPrice() - product.getBrokerage() *(1-MyApplication.vipRate/100.0));
                ...
                if (result.contains("1000") && result.contains("dataset")) {
                    try {
                        ...
                        for (int i = 0; i < dataset.length(); i++) {
                            ...
                            storeBean.setCreateTime(TimeFormatUtils.long2String(store.getLong("createTime")));
     ...
     * @param client
     */
    public static void add2shoppingcar(final BaseActivity context, String token, int skuId,int num, OkHttpClient client) {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.body().string().contains("1000")) {
                    ...
                    context.toastInUI(context.getApplicationContext(), "添加失败,可能选择了自己的商品");
                }
            }
        });
     ...
     * @param listener
     */
    public static void getMyRecommendStore(final BaseActivity context, String token, String url, int id,final int index, int size, OkHttpClient client, final OnGetMyRecommendStoreFinishedListener listener) {
        ...
        if (id==0) {
            ...
            builder.add("desired.id", id+"".trim());
        }
        FormBody body = builder.build();
        Request request = new Request.Builder()
                .post(body)
                    ...
                    try {
                        ...
                        for (int i = 0; i < dataset.length(); i++) {
                           ...
                           // storeBean.setLicence(jsonObject.getString("license"));
                            ...
                            storeBean.setCreateTime(TimeFormatUtils.long2String(jsonObject.getLong("createTime")));
                            if(jsonObject.has("user")){
                                JSONObject user = jsonObject.getJSONObject("user");
                                storeBean.setRecommender(user.getString("nickname"));
                            }
                            list.add(storeBean);
                        }
...
*/
                    ...
                    try {
                        ...
                        for (int i = 0; i < dataset.length(); i++) {
                           ...
                           /* if(jsonObject.has("data")){
                            ...
                            if(order.has("payTime")){
                                orderBean.setPayTime(order.getLong("payTime"));
                            }
                            //TODO 根据当前定位城市算出邮费
                            orderBean.setPostage(order.getInt("postage"));
                            ...
                            switch (status) {//TODO 对退款退货订单还需更细分类，待补充
                                //买家
                                case 0:
                                    orderBean.setState("等待付款");
                                    break;
                                case 1:
                                    orderBean.setState("等待卖家发货");
                                    break;
                                case 2:
                                    orderBean.setState("待收货");
                                    break;
                                case 3:
                                    orderBean.setState("等待评价");
                                    break;
                                case 4:
                            ...
                            if(!orderBean.isDeliver()){
                                if(orderBean.getStateNum()==1 && !orderBean.isDeliver()){
                                    ...
                                    orderBean.setState("商家已接单");
                                }else if(orderBean.getStateNum()==0 && !orderBean.isDeliver()){
                                    orderBean.setState("待付款(不支持发货)");
                                }
                                orderBean.setUnDeliverMan(order.getString("name"));
                                orderBean.setUnDeliverPhone(order.getString("phone"));
                            }
                ...
                if (result.contains("1000") && result.contains("dataset")) {
                    try {
                        ...
                        for (int i = 0; i < dataset.length(); i++) {
                            ...
                            orderBean.setBrokerage(productInfo.getInt("brokerage"));
                            orderBean.setSinglePrice(productInfo.getInt("price")-orderBean.getBrokerage()*(1-MyApplication.vipRate/100.0));
                            orderBean.setColor(productInfo.getString("content"));
                            orderBean.setSkuId(productInfo.getInt("id"));
     ...
     * @param listener
     */
    public static void getProductCS(OkHttpClient client, final int id, final OnGetProductCSFinishedListener listener) {
...
*/
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                String string = response.body().string();
                LogUtil.i("TAG","商品id"+id+"颜色尺寸==="+string);
                if (string.contains("1000")) {
                    try {
                        JSONObject object = new JSONObject(string);
                        JSONArray dataset = object.getJSONArray("dataset");
                ...
                if (string.contains("1000")) {
                    ...
                    try {
                       ...
                       // storeBean.setLicence(store.getString("license"));
                        ...
                        product.setCount(1);
                        product.setVipPrice(colorSizeBean.getPrice() - product.getBrokerage()*(1 - MyApplication.vipRate/100.0));
                        getPostage(product.getId(), new OnGetPostageFinishedListener() {
                            @Override
                            public void onGetPostageFinishedListener(double postage) {
                                if(postage==0){
    ...
    public static void getAllUnreadMsgCount(String token, final OnGetSomeCountFinishedListener listener){
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                String string = response.body().string();
                LogUtil.i("TAG","未读消息数量========="+string);
                if(string.contains("1000")){
                    try {
                        JSONObject object = new JSONObject(string);
                        JSONArray dataset = object.getJSONArray("data");
